/**
 *  \file   direct_diffusion_routing_functions.h
 *  \brief  AODV based routing approach: Auxiliary Functions Header File
 *  \author Luiz Henrique Suraty Filho (CEA-LETI)
 *  \date   May 2013
 **/

#ifndef  __direct_diffusion_routing_functions__
#define __direct_diffusion_routing_functions__
...
/** \brief Process a INTEREST packet received.
 ...
 *  \fn int set_direct_diffusion_header(call_t *to, packet_t *packet, int packet_type) 
 *  \param to is a pointer to the called class
 *  \param from is a pointer to the class who invoked
 *  \param packet is a pointer to the packet to be forwarded
 *  \param packet_type indicates which type of packet will be forwarded
 *  \return SUCCESSFUL if success, UNSUCCESSFUL otherwise
 **/
int set_direct_diffusion_header(call_t *to, call_t *from, packet_t *packet, int packet_type);
...
#endif  //__direct_diffusion_routing_functions__
