/**
 *  \file   half1d.c
 *  \brief  Halfduplex 1-d radio model without PHY header
...
/* ************************************************** */
model_t model =  {
    "Halfduplex radio interface without PHY header",
...
void radio_energy_update(call_t *to){
  ...
  //energy_consume(to, duration * nodedata->energy.current_draw_mA[nodedata->state]);
...
void radio_switch_state(call_t *to, int state){
  struct nodedata *nodedata = get_node_private_data(to);
...
int unbind(call_t *to) {
  ...
  printf("[RADIO_HALF1D] node %d consumption is %lf \n",to->object , sum_consumption);
...
void tx(call_t *to, call_t *from, packet_t *packet) {
  ...
  /* log tx */
  PRINT_TRANSCEIVER("[RADIO_HALF1D] node %d starts to transmit at time %"PRId64" \n",to->object, get_time());
...
void tx_end(call_t *to, call_t *from, packet_t *packet) {
  ...
  if (nodedata->tx_busy == packet->id) {
    PRINT_TRANSCEIVER("[RADIO_HALF1D] node %d ends to transmit at time %"PRId64" \n",to->object, get_time());
    nodedata->tx_busy = -1;
    radio_switch_state(to, RADIO_IDLE);
  }
...
void rx(call_t *to, call_t *from, packet_t *packet) {
  ...
  {
    PRINT_TRANSCEIVER("[RADIO_HALF1D] node %d received at time %"PRId64"\n",to->object, get_time());
    radio_switch_state(to, RADIO_IDLE);
...
void cs(call_t *to, call_t *from, packet_t *packet) {
  ...
  if ((nodedata->state != RADIO_RX) && (nodedata->state != RADIO_RXING)){
    PRINT_TRANSCEIVER("[RADIO_HALF1D] node %d make CS outside RX_STATE/RXING \n",to->object);
    return;
  }
